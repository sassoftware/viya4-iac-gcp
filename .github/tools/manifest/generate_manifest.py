...
def get_google_provider_version(root):
    """Extract the exact google provider version from versions.tf."""
    ...
    with open(versions_file, "r", encoding="utf-8") as f:
        content = f.read()
    # Match: google = { ... version = "6.28.0" ... } or google-beta = { ... version = "6.28.0" ... }
    match = re.search(
        r'google(?:-beta)?\s*=\s*\{[^}]*version\s*=\s*"([^"]+)"', content, re.DOTALL
    )
...
def extract_blocks(content):
    ...
    while i < len(lines):
        ...
        # Match: resource "google_*" "label" {
        res_match = re.match(r'\s*resource\s+"([^"]+)"\s+"[^"]+"\s*\{', line)
        # Match: data "google_*" "label" {
...
def main():
    ...
    version = get_google_provider_version(root)
    tf_files = find_tf_files(root)

    print(f"google provider version : {version}")
