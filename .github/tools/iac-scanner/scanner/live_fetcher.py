...
def fetch_google_provider_latest() -> Optional[str]:
    """Fetch the latest published google provider version from Terraform Registry.

    Used as a fallback when the manifest scanner has not been run.
    Returns None on failure.
    """
    data = _get_json("https://registry.terraform.io/v1/providers/hashicorp/google")
    if not data:
        return None
...
def build_live_seed(static_seed: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 2. google provider latest version (metadata — used when manifest_report is absent)
    google_provider_latest = fetch_google_provider_latest()
    if google_provider_latest:
        live["_live_google_provider_latest"] = google_provider_latest
        live["_fetch_status"]["google_provider"] = "live"
    else:
        live["_fetch_status"]["google_provider"] = "static_fallback"
    ...
    live_sources = [
        ...
        "https://registry.terraform.io/v1/providers/hashicorp/google",
