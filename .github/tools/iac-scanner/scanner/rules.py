...
def _build_rules() -> List[Rule]:
    ...
    # Old azurerm provider major versions (DISABLED FOR GCP)
    rules.append(Rule(
        id="TF001",
        category="terraform",
        ...
        enabled=False,  # Azure-specific, not applicable to GCP
    ))
    ...
    # Deprecated Terraform resource patterns (DISABLED FOR GCP)
    rules.append(Rule(
        id="TF006",
        category="terraform",
    ...
    rules.append(Rule(
        ...
        enabled=False,  # Disabled: High false positive rate on variable defaults and example values
    ))
